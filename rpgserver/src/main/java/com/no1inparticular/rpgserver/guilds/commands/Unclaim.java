...
public class Unclaim extends GuildCMD {

	public Unclaim(String description, String usage, String permission) {
	...
	public void execute(Player player, String[] args) {
		...
		if(guild != null) {
			if(guild.getOwner().equals(uuid)) {
				if (Main.guildManager.unclaimChunk(player.getLocation().getChunk(), guild)) {
	    			Bukkit.broadcastMessage(ChatColor.GREEN + "Chunk has been unclaimed.");
				} else {
	    			Bukkit.broadcastMessage(ChatColor.RED + "This chunk is not owned by your guild.");
				}
        	} else {
